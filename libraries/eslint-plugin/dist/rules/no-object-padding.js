    ...
    create(context) {
        return {
            ObjectExpression(node) {
                ...
                if (hasLeading) {
                    context.report({
                        fix: fixer => fixer.replaceTextRange([open.range[0] - 1, firstRange[0]], opening.replace(WHITESPACE_LINE, '')),
                ...
                if (hasTailing) {
                    context.report({
                        fix: fixer => fixer.replaceTextRange([lastRange[1] - 1, close.range[0]], closing.replace(WHITESPACE_LINE, '')),
